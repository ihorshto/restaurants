<x-app-layout>
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 px-4 space-y-6 mt-10">
        ...
        <form action="{{route('restaurants.store')}}" method="POST">
            ...
            <div class="grid md:grid-cols-2 gap-4">
                <div>
                    <div>
                        ...
                        <x-text-input id="name" class="block mt-1 w-full" type="text" name="name" :value="old('name')" placeholder="{{__('messages.restaurants.restaurant_name')}}" required />
                        <x-input-error :messages="$errors->get('name')" class="mt-2" />
                    </div>
                    <div class="sm:mt-6 mt-4">
                        <x-input-label for="description" :value="__('messages.description')" />
                        <textarea name="description" class="py-2 px-3 sm:py-3 sm:px-4 block w-full border-gray-200 rounded-lg sm:text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none" rows="3" placeholder="{{__('messages.restaurants.restaurant_description')}}">{{old('description')}}</textarea>
                    </div>
                </div>
            ...
            <button type="submit" class="button-default bg-black text-white">{{__('messages.create')}}</button>
        </form>
    </div>
</x-app-layout>
